...
(function(){
  ...
  function initializeModalInstances() {
    ...
    if (resetPasswordModalEl && !resetPasswordModal) {
      resetPasswordModal = new bootstrap.Modal(resetPasswordModalEl, modalOptions);
    ...
    if (modalsLoaded.accountForm) {
      ...
      if (roleSelect && !roleSelect.dataset.listenerAttached) {
        roleSelect.addEventListener('change', (e) => {
          ...
          if (rayonFields) {
            rayonFields.style.display = (role === 'koordinator' || role === 'jemaat') ? 'block' : 'none';
      ...
      const formInputs = document.querySelectorAll('#accountModal .form-control-modern, #accountModal .form-select');
  ...
  async function loadUsers(){
    try {
      ...
      if (!API || !API.users || !API.users.getAll) {
        throw new Error('API not properly loaded! Check if api.js is loaded before accounts.js');
      }
  ...
  function renderTable(users){
    ...
    if (!users || users.length === 0){
      ...
      tbody.innerHTML = '<tr><td colspan="6" class="text-center py-5"><div class="text-muted"><i class="bi bi-inbox" style="font-size: 3rem;"></i><p class="mt-3 mb-0 fw-semibold">Tidak ada akun ditemukan</p></div></td></tr>';
      return;
    }
    ...
    users.forEach((user, idx) => {
      ...
      const createdAt = user.createdAt || user.created_at ? new Date(user.createdAt || user.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }) : '-';
  ...
  async function openAddModal(){
    ...
    if (modalTitle) modalTitle.innerHTML = '<i class="bi bi-plus-circle me-2"></i>Tambah Akun Baru';
  ...
  async function openEditModal(id){
    ...
    const modalTitle = qs('accountModalLabel');
    if (modalTitle) modalTitle.innerHTML = '<i class="bi bi-pencil-square me-2"></i>Edit Akun';
    ...
    const rayonFields = document.getElementById('koordinator-fields');
    if (rayonFields) {
      rayonFields.style.display = (user.role === 'koordinator' || user.role === 'jemaat') ? 'block' : 'none';
    }
    ...
    if (user.role === 'jemaat') {
      ...
      if (passwordField) {
        ...
        passwordField.title = 'Password tidak dapat diubah oleh Admin untuk akun Jemaat';
      }
      ...
      if (passwordSection && !passwordSection.querySelector('.jemaat-edit-info')) {
        const infoDiv = document.createElement('div');
        infoDiv.className = 'alert alert-info mt-2 jemaat-edit-info';
        infoDiv.innerHTML = '<i class="bi bi-info-circle-fill me-2"></i><small><strong>Info:</strong> Untuk akun Jemaat, Admin hanya dapat mengubah <strong>Rayon</strong>. Email dan Password tidak dapat diubah.</small>';
  ...
  async function openDeleteModal(id){
    ...
    qs('delete-user-role-badge').innerHTML = getRoleBadge(user.role || 'jemaat');
    ...
    try {
      ...
      if (userToDelete.role === 'jemaat') {
        try {
          ...
          const toDelete = allPengajuan.filter(p => 
      ...
      await applyFilters();
      alert(`Akun ${userToDelete.nama || userToDelete.name || userToDelete.email} berhasil dihapus`);
    } catch (e) {
  ...
  async function saveAccount(){
    ...
    if ((role === 'koordinator' || role === 'jemaat') && !rayon) {
      alert('Rayon wajib diisi untuk ' + (role === 'koordinator' ? 'Koordinator' : 'Jemaat'));
      return;
    }
    ...
    try {
      ...
      await applyFilters();
      alert(currentEditId ? 'Akun berhasil diperbarui' : 'Akun berhasil ditambahkan');
    } catch (e) {
  ...
  async function confirmResetPassword() {
    ...
    try {
      ...
      const lsUser = (window.LS && typeof window.LS.getCurrentUser === 'function') ? window.LS.getCurrentUser() : null;
      const storedCurrent = !lsUser ? (JSON.parse(localStorage.getItem('currentUser') || 'null')) : null;
      const storedLegacy = !lsUser && !storedCurrent ? (JSON.parse(localStorage.getItem('user') || 'null')) : null;
      const currentUser = lsUser || storedCurrent || storedLegacy || {};

      const adminId = currentUser._id || currentUser.id || currentUser.user_id || null;
      const adminName = currentUser.name || currentUser.nama || currentUser.email || 'Admin';
