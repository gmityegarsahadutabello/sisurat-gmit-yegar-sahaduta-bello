...
const API = {
    ...
    for (let i = 0; i < API_BASE_URLS.length; i += 1) {
      ...
      try {
        ...
        if (isNetworkError(error) && i < API_BASE_URLS.length - 1) {
          console.warn(
            `⚠️ API unreachable at ${baseUrl}. Falling back to localhost.`,
          );
          continue;
        }
  ...
  pengajuan: {
    ...
    updateStatus: (id, statusData) =>
      API.request(`/pengajuan/${id}/status`, "PUT", statusData),
...
window.LS = {
  ...
  loadArray: async (key) => {
    try {
      ...
      throw new Error(
        `Failed to load ${key} from database. Please ensure backend is running.`,
      );
    }
  },
  ...
  saveArray: (key, arr) => {
    throw new Error(
      `LS.saveArray('${key}') is deprecated. Use API.${key}.create() or API.${key}.update() instead.`,
    );
  },
  ...
  logout: () => {
    ...
    const isLiveServer = /localhost:5500|127\.0\.0\.1:5500/i.test(
