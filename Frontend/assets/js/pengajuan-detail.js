...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function formatDate(d){
    try { const dt = new Date(d); return dt.toLocaleDateString('id-ID', { day:'2-digit', month:'long', year:'numeric' }) + ' ' + dt.toLocaleTimeString('id-ID'); }
    catch(e){ return d || '-'; }
  }
  ...
  function computeAge(item){
    ...
    const dobStr = item.tgl_lahir || (item.rawForm && item.rawForm.tgl_lahir) || item.rawForm && item.rawForm.ttl;
  ...
  async function loadData() {
    try {
      ...
      if (!data) {
        const local = await LS.find('local_pengajuan', it => String(it._id || it.id) === String(id));
        if (local) data = local;
        else {
      ...
      console.log('🔍 Loaded pengajuan data for', data.type || data.jenis, ':', {
      ...
      if (!data.pemohon_nama && data.user_nama) data.pemohon_nama = data.user_nama;
      ...
      if (typeof data.saksi1 === 'string') {
        try { data.saksi1 = JSON.parse(data.saksi1); } catch(e){ /* ignore */ }
      }
      if (typeof data.saksi2 === 'string') {
        try { data.saksi2 = JSON.parse(data.saksi2); } catch(e){ /* ignore */ }
      }
      // in some saved objects data for non-saksi forms may be inside rawForm
      if (!data.perihal && data.rawForm && data.rawForm.perihal) data.perihal = data.rawForm.perihal;
      if (!data.nama && data.rawForm && data.rawForm.nama) data.nama = data.rawForm.nama;
  ...
  function renderAddress(objPrefix, obj) {
    ...
    const tgl = obj.tgl_lahir ? formatDate(obj.tgl_lahir) : (obj.ttl ? escapeHtml(obj.ttl) : '-');
    return `
  ...
  function renderProgressIndicator(d) {
    ...
    const steps = [
      { key: 'submitted', label: 'Pengajuan Dibuat', icon: 'file-earmark-plus', desc: 'Surat pengajuan telah dibuat' },
      { key: 'verified', label: 'Verifikasi Koordinator', icon: 'person-check', desc: 'Menunggu verifikasi koordinator rayon' },
      { key: 'processed', label: 'Proses Tata Usaha', icon: 'file-earmark-text', desc: 'Diproses oleh tata usaha gereja' },
      { key: 'reviewed', label: 'Tinjauan Sekretaris & Pendeta', icon: 'people', desc: 'Ditinjau oleh sekretaris dan pendeta' },
      { key: 'completed', label: 'Selesai', icon: 'check-circle', desc: 'Surat selesai dan siap diambil' }
    ];
    ...
    if (statusLower.includes('ditolak') || statusLower.includes('rejected')) {
      ...
      if (statusLower.includes('rejected_by_koor') || statusLower === 'ditolak') {
    ...
    } else if (statusLower === 'validated_by_pendeta' || statusLower === 'validated' || statusLower === 'archived') {
      currentStep = 5; // All steps completed
    } else if (statusLower.includes('disposisi_to_pendeta') || statusLower.includes('reviewed_by_sekretaris')) {
      currentStep = 3;
    } else if (statusLower.includes('disposisi_to_sekretaris') || statusLower.includes('file_uploaded')) {
      currentStep = 2;
    } else if (statusLower === 'diterima' || statusLower === 'verified_by_koordinator' || statusLower === 'diverifikasi') {
      currentStep = 1;
    } else {
      currentStep = 0;
    }
        ...
        <div class="progress-steps">
          ${steps.map((step, idx) => {
            ...
            const timelineEntry = timeline.find(t => {
              ...
              if (idx === 0) return tNote.includes('dibuat') || tNote.includes('submitted') || tNote.includes('pengajuan dibuat');
              if (idx === 1) return tNote.includes('koordinator') || tNote.includes('verified') || tNote.includes('diterima');
              if (idx === 2) return tNote.includes('tata usaha') || tNote.includes('upload') || tNote.includes('disposisi');
              if (idx === 3) return tNote.includes('sekretaris') || tNote.includes('pendeta') || tNote.includes('review');
              if (idx === 4) return tNote.includes('validated') || tNote.includes('selesai') || tNote.includes('completed');
              return false;
            });
            
            if (timelineEntry) {
              const ts = timelineEntry.timestamp || timelineEntry.at || timelineEntry.tanggal || timelineEntry.waktu;
              if (ts) {
  ...
  function renderDetail(d) {
    ...
    document.getElementById('subtitle').textContent = `Jenis Surat: ${d.jenis || d.type || 'Tidak diketahui'}`;
    ...
    if (statusLower === 'ditolak' || statusLower === 'rejected' || statusLower === 'rejected_by_koor') {
    ...
    if (d.status === 'ditolak' || d.status === 'rejected_by_koor') {
      ...
      if (rejectionEntry) {
        ...
        if (!reason && rejectionEntry.keterangan) {
          const reasonMatch = rejectionEntry.keterangan.match(/Ditolak oleh Koordinator: (.+)/);
    ...
    const isFinished = (s === 'validated_by_pendeta' || s === 'validated' || s === 'archived' || d.validated === true);
    ...
    try { currentUser = window.LS && window.LS.getCurrentUser ? window.LS.getCurrentUser() : JSON.parse(localStorage.getItem('currentUser') || 'null'); } catch(e){ currentUser = null; }
    function isOwner(user, item){
      if (!user || !item) return false;
      const uid = user.id || user._id || user.nik || user.nik || user.nik;
      return String(uid) === String(item.user_id || item.user_id) || String(uid) === String(item.user_nik || item.pemohon_nik || item.nik);
    }
    ...
    console.debug('pengajuan-detail download check', { isFinished, fileUrl, currentUser: (currentUser && (currentUser.id||currentUser.nik)), showDownload });
    ...
    if (showDownload) {
      ...
      btnDownload.onclick = async function(e){
        ...
        try {
          ...
          const url = detail && (detail.file_url || detail.final_file_url || detail.file_url_final || detail.downloadUrl || detail.final_file || detail.final_file_data);
          if (!url) {
            alert('File belum tersedia untuk diunduh. Silakan coba kembali nanti.');
            return;
          }
    ...
    switch ((d.type||d.jenis||'').toLowerCase()) {
      ...
      case 'surat lainnya':
        ...
        const hasDraftFile = d.draft_file_url || (d.files && d.files.draft && d.files.draft.key);
        
        // Check for supporting files - try both form and top level
        let hasSupFiles = false;
        if (d.form) {
          hasSupFiles = !!(
            (d.form.file_sup1 && (d.form.file_sup1.key || d.form.file_sup1_url)) ||
            (d.form.file_sup2 && (d.form.file_sup2.key || d.form.file_sup2_url)) ||
            (d.form.file_sup3 && (d.form.file_sup3.key || d.form.file_sup3_url)) ||
        ...
        if (hasDraftFile || hasSupFiles) {
          ...
          if (hasDraftFile) {
            ...
            html += renderField('File utama', `<a href="${escapeHtml(draftUrl)}" target="_blank" class="text-primary"><i class="bi bi-file-pdf"></i> ${escapeHtml(draftName)}</a>`, 'file-pdf');
          }
