...
(function(){
  ...
  const confirmVerifySpinner = document.getElementById('confirm-verify-spinner');
  ...
  const confirmRejectSpinner = document.getElementById('confirm-reject-spinner');
  ...
  function renderAddress(obj){
    if(!obj || Object.keys(obj).length===0) return renderField('Alamat','-');
    return [
      renderField('Nama lengkap', escapeHtml(obj.nama || obj.nama_lengkap || obj.namaPemohon || '-')),
      renderField('Tempat, Tgl Lahir', escapeHtml(obj.ttl || obj.ttl_raw || obj.tempat_tgl_lahir || '-')),
      renderField('Jenis Kelamin', escapeHtml(obj.jk || '-')),
      renderField('Agama', escapeHtml(obj.agama || '-')),
      renderField('Alamat', `${escapeHtml(obj.jalan||'')} (RT ${escapeHtml(obj.rt||'')} / RW ${escapeHtml(obj.rw||'')})`),
  ...
  async function loadAndRender(){
    ...
    const list = all.filter(p => {
      // rayon constraint
      if (rayon && p.rayon && String(p.rayon) !== String(rayon) && !String(p.rayon).includes(String(rayon))) return false;
      ...
      const excludedStatuses = [
        'diterima','diverifikasi','terverifikasi','ditolak',
        'disposisi_to_sekretaris','disposisi_to_pendeta','disposisi_to_tatausaha',
        'verified_by_koordinator','surat_dibuat',
        'validated_by_pendeta', 'validated', 'archived'
      ];
      if (excludedStatuses.includes(s)) return false;
      // Also exclude if verified_by_koordinator flag is set
      if (p.verified_by_koordinator === true) return false;
  ...
  function renderTable(list){
    ...
    list.forEach((it, idx) => {
      ...
      const jenis = escapeHtml(flatItem.jenis || flatItem.type || flatItem.tipe || '-');
      const perihal = escapeHtml(flatItem.perihal || flatItem.ringkasan || flatItem.keterangan || '-');
      const tanggal = fmtDateTime(flatItem.created_at || flatItem.createdAt || flatItem.tanggal || flatItem.date || '-');
      const statusLabel = (flatItem.status||'proses').toLowerCase();
      const statusHtml = statusLabel === 'proses' ? '<span class="badge-status-proses">Diproses</span>' : (statusLabel === 'ditolak' ? '<span class="badge-status-ditolak">Ditolak</span>' : '<span class="badge-status-diterima">Diverifikasi</span>');
  ...
  async function openDetail(id){
    ...
    const statusClass = item.status === 'pending' ? 'status-pending' : 
                       item.status === 'diterima' || item.status === 'approved' ? 'status-approved' : 
                       item.status === 'verified' ? 'status-verified' : 'status-pending';

    // Card 1: Informasi Akun Pemohon
    html += `<div class="detail-card">`;
    html += `<div class="detail-section"><i class="bi bi-person-circle"></i> Informasi Pemohon</div>`;
    html += renderField('Nama Lengkap', escapeHtml(item.user_nama || item.pemohon_nama || item.nama || '-'));
    html += renderField('NIK', escapeHtml(item.user_nik || item.pemohon_nik || item.nik || '-'));
    html += renderField('Email', escapeHtml(item.user_email || item.email || item.pemohon_email || '-'));
    html += renderField('Rayon', escapeHtml(item.rayon || item.user_rayon || getRayon() || '-'));
    html += renderField('Tanggal Pengajuan', escapeHtml(fmtDateTime(item.created_at || item.createdAt || item.tanggal || item.date)));
    html += renderField('Status', `<span class="detail-badge ${statusClass}">${escapeHtml(item.status || '-')}</span>`);
    html += `</div>`;

    // Card 2: Detail Surat
    html += `<div class="detail-card">`;
    html += `<div class="detail-section"><i class="bi bi-file-earmark-text"></i> Detail Surat</div>`;
    html += renderField('Jenis Surat', escapeHtml(item.jenis || item.type || item.tipe || '-'));
    html += renderField('Perihal / Ringkasan', escapeHtml(item.perihal || item.ringkasan || item.keterangan || '-'));
    html += `</div>`;
    ...
    if(t.includes('saksi')){
      ...
      html += renderField('Nama', `<strong>${escapeHtml(s1.nama || '-')}</strong>`);
      ...
      html += renderField('Tempat, Tgl Lahir', `${s1TempatLahir}, ${s1TglLahir}`);
      ...
      if (s1.rt || s1.rw) s1AlamatParts.push(`RT ${escapeHtml(s1.rt||'-')}/RW ${escapeHtml(s1.rw||'-')}`);
      ...
      const s1Alamat = s1AlamatParts.length > 0 ? s1AlamatParts.join(', ') : '-';
      ...
      if(item.saksi_count === 2 || item.saksi2 || item.saksi_2){
        ...
        html += renderField('Nama', `<strong>${escapeHtml(s2.nama || '-')}</strong>`);
        
        const s2TempatLahir = escapeHtml(s2.tempat_lahir || '-');
        // Keep full date format YYYY-MM-DD
        const s2TglLahir = s2.tgl_lahir || (s2.ttl ? escapeHtml(s2.ttl) : '-');
        html += renderField('Tempat, Tgl Lahir', `${s2TempatLahir}, ${s2TglLahir}`);
        ...
        if (s2.rt || s2.rw) s2AlamatParts.push(`RT ${escapeHtml(s2.rt||'-')}/RW ${escapeHtml(s2.rw||'-')}`);
        if (s2.kelurahan) s2AlamatParts.push(`Kel. ${escapeHtml(s2.kelurahan)}`);
        if (s2.kecamatan) s2AlamatParts.push(`Kec. ${escapeHtml(s2.kecamatan)}`);
        if (s2.kota) s2AlamatParts.push(escapeHtml(s2.kota));
        const s2Alamat = s2AlamatParts.length > 0 ? s2AlamatParts.join(', ') : '-';
      ...
      if (rkRt || rkRw) rkAlamatParts.push(`RT ${escapeHtml(rkRt||'-')}/RW ${escapeHtml(rkRw||'-')}`);
      ...
      html += renderField('Keperluan / Untuk', escapeHtml(item.untuk || item.keperluan || '-'));
      html += `</div>`; // end card
    }
    ...
    // Catatan
    html += renderField('Catatan', escapeHtml(item.catatan || item.note || item.keterangan || '-'));
    html += `</div>`; // end card
    ...
    // show modal
    try { if (typeof dialog.showModal === 'function') dialog.showModal(); else alert(detailBody.textContent || JSON.stringify(item)); } catch(e) { alert('Tidak dapat membuka modal detail'); }
  }
  ...
  function showInlineMessage(text, type='info'){
    ...
    el.style.cssText = 'position:fixed;right:18px;bottom:18px;padding:10px 14px;border-radius:8px;background:#222;color:#fff;z-index:1200;opacity:1;transition:opacity .35s';
    el.textContent = text;
  ...
  document.addEventListener('click', async (e) => {
    ...
    if(action === 'periksa'){ await openDetail(id); return; }
    
    if(action === 'verifikasi'){
      pendingActionId = id;
      let it = null;
      try { it = await API.pengajuan.getById(id); } catch(e){}
      ...
      ConfirmModal.show({
        ...
        detail: {
          ...
          'Tanggal Pengajuan': fmtDateTime(it?.created_at || it?.createdAt) || '-'
        },
    ...
    if(action === 'tolak'){
      ...
      ConfirmModal.show({
        ...
        inputMinLength: 10,
        showWarning: true,
        warningText: 'Pemohon akan menerima notifikasi penolakan beserta alasan yang Anda tuliskan.',
  ...
  confirmVerifyCancel?.addEventListener('click', ()=> { try{ confirmVerifyModal.close(); }catch(e){} pendingActionId = null; });
  confirmVerifyYes?.addEventListener('click', async ()=> {
    if(!pendingActionId) return;
    confirmVerifySpinner && (confirmVerifySpinner.style.display='inline-block');
    confirmVerifyLabel && (confirmVerifyLabel.style.opacity='0.6');
    confirmVerifyYes.setAttribute('disabled','disabled');
    try { await Promise.resolve(changeStatus(pendingActionId,'diterima')); } finally {
      ...
      try{ confirmVerifyModal.close(); }catch(e){}
      try{ dialog.close(); }catch(e){}
    }
  });

  confirmRejectCancel?.addEventListener('click', ()=> { try{ confirmRejectModal.close(); }catch(e){} pendingActionId = null; });
  confirmRejectYes?.addEventListener('click', async ()=> {
    const note = (rejectReasonEl.value || '').trim();
    if(!note){ rejectReasonEl.focus(); rejectReasonEl.style.boxShadow='0 0 0 3px rgba(220,53,69,0.12)'; setTimeout(()=> rejectReasonEl.style.boxShadow='',1200); return; }
    if(!pendingActionId) return;
    confirmRejectSpinner && (confirmRejectSpinner.style.display='inline-block');
    confirmRejectLabel && (confirmRejectLabel.style.opacity='0.6');
    confirmRejectYes.setAttribute('disabled','disabled');
    try { await Promise.resolve(changeStatus(pendingActionId,'ditolak',note)); } finally {
      ...
      pendingActionId = null;
      try{ confirmRejectModal.close(); }catch(e){}
      try{ dialog.close(); }catch(e){}
    }
  });
