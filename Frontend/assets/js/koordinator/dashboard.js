...
(function() {
  ...
  async function loadDashboardData() {
    ...
    if (elRole) elRole.textContent = rayon ? `Koordinator ${rayon}` : 'Koordinator Rayon';
    ...
    const filteredSubmissions = targetRayon ? allSubmissions.filter(item => {
      // Check direct rayon property
      const itemRayon = String(item.rayon || item.user_rayon || '').toLowerCase();
      if (itemRayon && (itemRayon === targetRayon || itemRayon.includes(targetRayon) || targetRayon.includes(itemRayon))) {
        return true;
      }

      // Resolve via user list if rayon missing on item
      const applicant = users.find(u => 
        (u.nik && item.user_nik && String(u.nik) === String(item.user_nik)) ||
      ...
      if (applicant && applicant.rayon) {
        ...
        return userRayon === targetRayon || userRayon.includes(targetRayon) || targetRayon.includes(userRayon);
      }
    ...
    filteredSubmissions.forEach(item => {
      ...
      const isMasuk = !excludedStatuses.includes(s) && item.verified_by_koordinator !== true;
      ...
      if (isMasuk) {
        ...
        if (!itemVerifiedRayon || !targetRayon || itemVerifiedRayon === targetRayon) {
